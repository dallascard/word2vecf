...
struct vocabulary *wv;  // word vocabulary
struct vocabulary *cv;  // context vocabulary

int negative = 15;      // # negative samples ? (k)
...
void *TrainModelThread(void *id) {
  ...
  unsigned long long next_random = (unsigned long long)id;          // get a random index (?)
  real f, g;
  clock_t now;
  ...
  for (iter=0; iter < numiters; ++iter) {
     ...
     while (1) { //HERE @@@
        ...
        if (word_count - last_word_count > 10000) {
           ...
           alpha = starting_alpha * (1 - word_count_actual / (real)(numiters*train_words + 1));     // alpha = learning rate
           if (alpha < starting_alpha * 0.0001) alpha = starting_alpha * 0.0001;
        }
        if (feof(fi) || ftell(fi) > end_offset) break;
        for (c = 0; c < layer1_size; c++) neu1[c] = 0;  // set new vectors to 0
        for (c = 0; c < layer1_size; c++) neu1e[c] = 0;

        // read a word from the file and get its index in the word vocabulary
        wrdi = ReadWordIndex(wv, fi);
        // read the next word in the file and get its index in the context vocabulary
        ctxi = ReadWordIndex(cv, fi);
        word_count++; //TODO ?

        if (wrdi < 0 || ctxi < 0) continue;

        // do some kind of subsampling?
        if (sample > 0) {
        ...
        l1 = wrdi * layer1_size;        // index into [vocab * layer size] array for word
        for (d = 0; d < negative + 1; d++) {
           if (d == 0) {
              // on first epoch, use given context, and d=1
              target = ctxi;
              label = 1;
           } else {
              // on all others, sample a random context and use d=0
              next_random = next_random * (unsigned long long)25214903917 + 11;
           ...
           l2 = target * layer1_size;   // index into [vocab * layer size] array for current context
           f = 0;
           // dot product of word and target
           for (c = 0; c < layer1_size; c++) f += syn0[c + l1] * syn1neg[c + l2];
           // if f is too large, set gradient to label * learning rate ?
           if (f > MAX_EXP) g = (label - 1) * alpha;
           // similarly if dot product is too small
